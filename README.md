...
## Добавление команд
  ...
  ```
  - зарегистрировать клас необходимо следующим образом - в методе `initialize` класса `App` необходимо вставить следующее:
 ...
 - метод `call(item=nil)` возвращает результат работы вашего класса(команды), так же может принимать один аргумент,
  который по умолчанию является `nil`. Добавление других методов в код вашего класса, только на ваше усмотрение,
